...
/* Out-of-tree additions (uwbl project) */
#define UNIWILL_FEATURE_FAN_MODE		BIT(15)
#define UNIWILL_FEATURE_FAN_BOOST		BIT(16)
...
struct uniwill_data {
	...
	struct mutex fan_lock;		/* Protects fan mode / fan boost toggling */
...
/*
 * Out-of-tree addition: "Fan Boost" toggle, equivalent to the Fan Boost button of the
 * OEM Control Center. Like the super key lock it can only be toggled via the trigger
 * register, the current state is reported in the switch status register.
 */
static int uniwill_read_fan_boost(struct uniwill_data *data, bool *status)
{
	unsigned int value;
	int ret;
	...
	*status = !!(value & FAN_BOOST_STATUS);

	return 0;
}

static int uniwill_write_fan_boost(struct uniwill_data *data, bool enable)
{
	bool status;
	int ret;
	...
	ret = uniwill_read_fan_boost(data, &status);
	if (ret < 0)
		return ret;
	...
	return regmap_write_bits(data->regmap, EC_ADDR_TRIGGER, TRIGGER_FAN_BOOST,
				 TRIGGER_FAN_BOOST);
}
			       ...
			       const char *buf, size_t count)
{
	...
	ret = uniwill_write_fan_boost(data, enable);
	if (ret < 0)
		return ret;
...
static umode_t uniwill_attr_is_visible(struct kobject *kobj, struct attribute *attr, int n)
{
	...
	if (attr == &dev_attr_fan_boost.attr) {
		if (uniwill_device_supports(data, UNIWILL_FEATURE_FAN_BOOST))
			return attr->mode;
	}
...
/*
 * Out-of-tree addition: fan/performance mode via the platform profile interface.
 *
 * The OEM Control Center exposes three modes (Office / Gaming / Beast). "Gaming" is the
 * EC default (manual fan control register cleared), "Beast" sets the turbo bit. "Office"
 * (silent) is implemented by the EC firmware behind the silent mode trigger, which is not
 * reliably readable back on all firmware, so only balanced/performance are offered here.
 */
...
static int uniwill_platform_profile_set(struct device *dev, enum platform_profile_option profile)
{
	...
	return regmap_update_bits(data->regmap, EC_ADDR_MANUAL_FAN_CTRL, FAN_MODE_TURBO, value);
}
...
static int uniwill_platform_profile_init(struct uniwill_data *data)
{
	...
	if (!uniwill_device_supports(data, UNIWILL_FEATURE_FAN_MODE))
		return 0;

	ppdev = devm_platform_profile_register(data->dev, DRIVER_NAME, data,
					     ...
					     enum led_brightness brightness)
{
	...
	for (int i = 0; i < KBD_LED_CHANNELS; i++) {
		/*
		 * Older LED cores do not validate multi_intensity against a per-channel
		 * maximum, so clamp here to keep the EC happy.
		 */
		regval = clamp(data->kbd_led_mc_subled_info[i].intensity, min_intensity,
			       (unsigned int)KBD_LED_MAX_INTENSITY);
...
#ifdef UW_HAVE_MC_MAX_INTENSITY
...
/*
 * Eluktronics RP-17 (Tongfang GK7NR0R, same chassis as the TUXEDO Polaris 17 Gen1 AMD).
...
static struct uniwill_device_descriptor eluktronics_rp17_descriptor __initdata = {
	...
	.features = UNIWILL_FEATURE_FN_LOCK |
		    ...
		    UNIWILL_FEATURE_FAN_MODE |
		    UNIWILL_FEATURE_FAN_BOOST,
...
static const struct dmi_system_id uniwill_dmi_table[] __initconst = {
	{
		.ident = "Eluktronics RP-17",
		.matches = {
			DMI_MATCH(DMI_SYS_VENDOR, "Eluktronics"),
			DMI_EXACT_MATCH(DMI_PRODUCT_NAME, "RP-17"),
		},
		.driver_data = &eluktronics_rp17_descriptor,
	},
	{
