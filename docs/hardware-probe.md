...
| OS / kernel | KDE neon 24.04, `7.0.0-30-generic` |
...
| 6 | Fn + brightness key | `brightness_hw_changed` updates, KDE OSD appears, `uwbl status` follows | |
| 7 | `/sys/firmware/acpi/platform_profile_choices` | `balanced performance` | |
| 8 | `uwbl fan performance` | fans audibly ramp (Beast) | |
| 9 | `uwbl boost on` | `fan_boost` reads 1, fans at max | |
...
| 13 | Reboot | module autoloads, `uwbld` active, tray autostarts, last settings restored | |
